...
class Document:
    ...
    def get_text(self, get_header_text=False, get_footer_text=False, image_dir=None, extensions=None, lltid=None):
        """
        ...
        for f in zip_f.namelist():
            if get_header_text and f.startswith('word/header', tid):
                text += self.xml_to_text(zip_f.read(f))
            if f.startswith('word/document'):
                text += self.xml_to_text(zip_f.read(f), lltid, listn)
            if get_footer_text and f.startswith('word/footer'):
                text += self.xml_to_text(zip_f.read(f), tid)
    ...
    def xml_to_text(self, xml, lltid, listn):
        """
        ...
        for child in root.iter():
            ...
            if child.tag == qn('w:t'):
                ...
                text += f'</br><img class="imj" src="file:///root/cardbot/pythonProject/bot/img/id{lltid}/{listn[n_images]}" alt="Girl in a jacket" width="234px">\n'
                n_images += 1
