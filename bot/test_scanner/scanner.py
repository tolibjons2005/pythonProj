...
from db.user import get_st_answ, add_score, add_ans_message,get_answers_oo,add_to_dict, is_registered
...
def memoize(func):
    ...
    return wrapper
async def test_scanner_fo(imgg,st_id, session_maker):
    ...
    print(userid)
    id = userid[3:]
    test_type = userid[1:3]
    data,fullname = await get_answers_oo(int(id), test_type, session_maker, st_id)

    if data == None:

        resultg = 'Ushbu test uchun ajratilgan vaqt yakunlangan😔'

        return resultg


    elif data.data != None and str(st_id) in data.data:
        resultg = 'Ushbu test siz tomoningizdan oldin tekshirilgan'
        ...
        if biggestContour.size != 0:
            ...
            max_val = np.amax(myPixelVal) - 200.0
        ...
        await add_to_dict(int(id),st_id,str(resultg)+ '%'+fullname+ '%'+str(datNow),session_maker)
